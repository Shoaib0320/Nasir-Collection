...
// import { ListAlt } from '@mui/icons-material'; // Import icons
...
//                       }} 
//                     sx={{ '&:hover': { backgroundColor: '#008080' , color: '#fff' }, color: '#444' , fontStyle: 'italic' }}
//                     to="/customersorder">
//                       <MenuItem >
//                         {/* <Typography variant="body3" sx={{ '&:hover': { backgroundColor: '#008080' , color: '#fff' , padding:'opx 10px' }, color: '#444' , fontStyle: 'italic' }}> */}
//                         {/* </Typography> */}                          
...
export const Navbar = () => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        const checkoutRef = doc(db, 'Checkout', currentUser.displayName);
        const docSnapshot = await getDoc(checkoutRef);
        setHasOrders(docSnapshot.exists() && docSnapshot.data().checkouts.length > 0);
      } else {
        setHasOrders(false);
      }
    });
                ...
                >
                  ...
                  {hasOrders && (
                    <MenuItem
                      ...
                      sx={{ display: 'flex', alignItems: 'center', gap: '10px', color: '#000' }}
                    >
                      <ListAlt />
                      <span>Your Orders</span>
                    </MenuItem>
                  ...
                  <Divider />
                  <MenuItem onClick={handleLogout} sx={{ fontWeight: 'bold', color: '#444' }}>
                    Logout
                  </MenuItem>
                </StyledMenu>
              </div>
